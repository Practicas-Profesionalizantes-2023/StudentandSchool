...
if(isset($_GET['id'])){
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        $fk_career_id = isset($_POST["fk_career_id"]) ? $_POST["fk_career_id"]: null;
        $fk_id_gender = isset($_POST["id_gender"]) ? $_POST["id_gender"]: null;


        // Llama a la función de actualización de la base de datos
        $updated = $database->updateStudent($id_student, $name, $last_name, $direction,$height, $uk_dni, $email,$phone,$birth_date,$fk_career_id,$fk_id_gender);
    
        if ($updated) {
            // La actualización fue exitosa, redirige a una página de éxito o muestra un mensaje de éxito
            header("Location: ../views/views_students.php");
            exit;
        } else {
